...
import { useWeightedAvailabilityRecommendations } from '../../api/mfc';
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300 text-[12px]">
          ...
          Editing here is not wired up yet — manage live prices from Recommend Prices. Prices are shared with Ultrafresh Availability, not separate per tool.
        </div>
...
export default function WeightedAvailabilityRecommendationsPage() {
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [editingProduct, setEditingProduct] = useState(null);
  const { data, isLoading, isError, error, refetch } = useWeightedAvailabilityRecommendations();
